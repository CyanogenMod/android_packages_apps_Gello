...
TOP_SWE=$(realpath .)
SRC_DIR=$TOP_SWE/src
BUILD_DIR=$TOP_SWE/build
GCLIENT_FILE=$TOP_SWE/.gclient
DONE_FILE=$TOP_SWE/.cm_done

GELLO_SRC=$TOP_SWE/gello
...
function build_gello() {
    ...
    if [ "$BUILDRETURN" == 0 ]; then
        if [ -f "$COMPILED_APK" ]; then
            READY_APK=$TOP_SWE/Gello.apk &> /dev/null
            if [ -f "$READY_APK" ]; then
                rm -rf $READY_APK
            fi
...
function sync_swe() {
    if [ -f "$DONE_FILE" ]; then
        ...
        # Replace src/swe/browser (gello) with
        # src/swe/browser-caf (upstream) so we can fetch
        # with no probs from caf
        rm -rf $BUILD_BROWSER &>/dev/null
        if [ -d "$CAF_BROWSER" ]; then
            mv $CAF_BROWSER $BUILD_BROWSER
        else
            echo "W: Unable to find CAF Browser! Sync will continue anyway..."
        fi
        # Start now
        echo "Syncing now"
        ...
        # First Sync
        echo "Syncing for the first time, this may take much time!"
        echo "About 16gb of sources will be downloaded, if you prefer to use prebuilt apk use:"
        echo "    export WITH_GELLO_PREBUILT=true"
        read -p "Do you want to download all the sources? [y/N]: " CHOICE
        # Check user preference
        # If needed (eg jenkins) it's possible to export FORCE_GELLO_SOURCE=true to skip this
        if [ "$CHOICE" == "y" ] || [ "$FORCE_GELLO_SOURCE" == "true" ]; then
            gclient sync -n --no-nag-max
            local SYNCRETURN=$?
            if [ "$SYNCRETURN" == 0 ]; then
                # Everything was fine, let's move gello to its home
            ...
            # user wants to use prebuilt or doesn't want to syng everything
            echo "You've choosen to not download the sources!"
            exit 2
        fi
    fi
}
